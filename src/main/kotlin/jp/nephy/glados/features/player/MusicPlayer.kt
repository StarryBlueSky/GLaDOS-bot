...
class MusicPlayer: BotFeature() {
    ...
    @Command(channelType = Command.ChannelType.TextChannel, description = "指定されたメディアを再生します。", args = ["検索ワード|動画URL|プレイリストURL"], category = "Music Bot", checkArgsCount = false)
    ...
    @Command(channelType = Command.ChannelType.TextChannel, description = "ニコニコ動画のランキングを再生します。", category = "Music Bot")
    ...
    @Command(aliases = ["sc"], channelType = Command.ChannelType.TextChannel, description = "SoundCloudのチャートを再生します。", category = "Music Bot")
    ...
    @Command(channelType = Command.ChannelType.TextChannel, description = "現在の再生キューを取得します。", category = "Music Bot")
    ...
    @Command(channelType = Command.ChannelType.TextChannel, description = "現在接続中のボイスチャンネルにGLaDOSを呼びます。", category = "Music Bot")
    fun summon(event: CommandEvent) {
        val guildPlayer = event.guild?.player ?: return

        reject(event.member!!.voiceState?.channel == guildPlayer.currentVoiceChannel) {
            event.embedError("!summon") {
