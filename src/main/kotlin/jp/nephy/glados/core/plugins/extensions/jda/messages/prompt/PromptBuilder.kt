...
import jp.nephy.glados.core.plugins.extensions.jda.messages.wait
...
class PromptBuilder(val channel: MessageChannel, val target: Member) {
    inline fun <T: PromptEmoji, reified R: Enum<T>> emoji(
        ...
        channel.reply(target) {
            ...
            GLaDOS.eventWaiter.wait<GuildMessageReactionAddEvent>({ user.idLong == target.user.idLong && messageIdLong == m.idLong && !enumConstants.none { it.emoji == reactionEmote.name } }, timeoutSec?.times(1000L), { m.delete().launch() }) {
    ...
    inline fun <T: PromptEnum, reified R: Enum<T>> enum(
        ...
        channel.reply(target) {
            ...
            GLaDOS.eventWaiter.wait<GuildMessageReceivedEvent>({
