package jp.nephy.glados.core.audio.player.api.niconico
...
import jp.nephy.glados.core.audio.player.api.niconico.param.RankingCategory
import jp.nephy.glados.core.audio.player.api.niconico.param.RankingPeriod
import jp.nephy.glados.core.audio.player.api.niconico.param.RankingType
import jp.nephy.glados.core.audio.player.api.niconico.param.SearchTarget
...
class NiconicoClient {
    ...
    suspend fun search(query: String, target: SearchTarget = SearchTarget.Title, limit: Int = 10): SearchResult {
        val url =
            "http://api.search.nicovideo.jp/api/v2/snapshot/video/contents/search?q=${query.encodeURLParameter()}&targets=${target.name.toLowerCase()}&fields=contentId,title,description,tags,categoryTags,viewCounter,mylistCounter,commentCounter,lengthSeconds&_sort=-viewCounter&_limit=$limit&_context=GLaDOS"
        return get(url)
    }
}
