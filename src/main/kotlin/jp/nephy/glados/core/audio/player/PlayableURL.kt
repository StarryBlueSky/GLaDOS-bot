...
enum class PlayableURL(val friendlyName: String, val url: String, val faviconUrl: String, val color: HexColor, vararg val regexes: Regex) {
    ...
    Niconico(
        "ニコニコ動画", "http://www.nicovideo.jp", "https://nicovideo.cdn.nimg.jp/web/img/favicon.ico", HexColor.Niconico, "(?:http(?:s)?://)?(?:www\\.|sp\\.|m\\.)?nicovideo\\.jp/watch/sm\\d+".toRegex(), "(?:http(?:s)?://)?nico\\.ms/sm\\d+".toRegex(), "sm\\d+".toRegex()
    ),
    SoundCloud(
        "SoundCloud",
    ...
    Twitch(
        "Twitch", "https://www.twitch.tv", "https://static.twitchcdn.net/assets/favicon-75270f9df2b07174c23ce844a03d84af.ico", HexColor.Twitch, "(?:http(?:s)://)?(?:www\\.|go\\.)?twitch.tv/(?:[^/]+)".toRegex()
    );
