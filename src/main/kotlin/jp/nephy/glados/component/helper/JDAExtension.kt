...
fun MessageAction.deleteQueue(delay: Long? = null, unit: TimeUnit = TimeUnit.MILLISECONDS, cacheManager: MessageCacheManager? = null, then: (Message) -> Unit = {  }) {
    queue {
        if (delay != null) {
            thread {
                unit.sleep(delay)
                if (cacheManager == null || cacheManager.contains(it.idLong)) {
                    it.delete().submit()
                }
            }
        } else {
