...
object Audit: Plugin() {
    ...
    override suspend fun onCategoryCreate(event: CategoryCreateEvent) {
        messageLog(event.guild, "カテゴリー 作成", HexColor.Good) { "`${event.category.name}` が作成されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onCategoryDelete(event: CategoryDeleteEvent) {
        messageLog(event.guild, "カテゴリー 削除", HexColor.Bad) { "`${event.category.name}` が削除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onCategoryUpdateName(event: CategoryUpdateNameEvent) {
        messageLog(event.guild, "カテゴリー 名前変更", HexColor.Change) { "`${event.oldName}` から `${event.category.name}` に名前が変更されました。" }
    }
    ...
    override suspend fun onTextChannelCreate(event: TextChannelCreateEvent) {
        messageLog(event.guild, "テキストチャンネル 作成", HexColor.Good) { "`${event.channel.name}` が作成されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onTextChannelDelete(event: TextChannelDeleteEvent) {
        messageLog(event.guild, "テキストチャンネル 削除", HexColor.Bad) { "`${event.channel.name}` が削除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onTextChannelUpdateName(event: TextChannelUpdateNameEvent) {
        messageLog(event.guild, "テキストチャンネル 名前変更", HexColor.Neutral) { "`${event.oldName}` から `${event.channel.name}` に名前が変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onTextChannelUpdateNSFW(event: TextChannelUpdateNSFWEvent) {
        if (event.channel.isNSFW) {
            messageLog(event.guild, "テキストチャンネル NSFW有効化", HexColor.Bad) { "`${event.channel.name}` にNSFWフラグが設定されました。" }
        } else {
            messageLog(event.guild, "テキストチャンネル NSFW無効化", HexColor.Good) { "`${event.channel.name}` からNSFWフラグが解除されました。" }
        }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onTextChannelUpdateParent(event: TextChannelUpdateParentEvent) {
        messageLog(event.guild, "テキストチャンネル 所属カテゴリー変更", HexColor.Neutral) { "`${event.channel.name}` は カテゴリー `${event.oldParent?.name ?: "(カテゴリーなし)"}` から `${event.channel.parent?.name ?: "(カテゴリーなし)"}` に移動しました。" }
    }
    ...
    override suspend fun onTextChannelUpdateTopic(event: TextChannelUpdateTopicEvent) {
        ...
        messageLog(event.guild, "テキストチャンネル トピック変更", HexColor.Neutral) {
            if (event.oldTopic.isNullOrEmpty() && !event.channel.topic.isNullOrEmpty()) {
                "`${event.channel.name}` のトピックが `${event.channel.topic}` に設定されました。"
            } else if (!event.oldTopic.isNullOrEmpty() && event.channel.topic.isNullOrEmpty()) {
    ...
    override suspend fun onVoiceChannelCreate(event: VoiceChannelCreateEvent) {
        messageLog(event.guild, "ボイスチャンネル 作成", HexColor.Good) { "`${event.channel.name}` が作成されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onVoiceChannelDelete(event: VoiceChannelDeleteEvent) {
        messageLog(event.guild, "ボイスチャンネル 削除", HexColor.Bad) { "`${event.channel.name}` が削除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onVoiceChannelUpdateName(event: VoiceChannelUpdateNameEvent) {
        messageLog(event.guild, "ボイスチャンネル 名前変更", HexColor.Neutral) { "`${event.oldName}` から `${event.channel.name}` に名前が変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onVoiceChannelUpdateParent(event: VoiceChannelUpdateParentEvent) {
        messageLog(event.guild, "ボイスチャンネル 所属カテゴリー変更", HexColor.Neutral) { "`${event.channel.name}` は カテゴリー `${event.oldParent?.name ?: "(カテゴリーなし)"}` から `${event.channel.parent?.name ?: "(カテゴリーなし)"}` に移動しました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onVoiceChannelUpdateBitrate(event: VoiceChannelUpdateBitrateEvent) {
        messageLog(event.guild, "ボイスチャンネル ビットレート変更", HexColor.Neutral) { "`${event.channel.name}` のビットレートが `${event.oldBitrate} kbps` から `${event.channel.bitrate} kbps` に変更されました。" }
    }
    ...
    override suspend fun onVoiceChannelUpdateUserLimit(event: VoiceChannelUpdateUserLimitEvent) {
        messageLog(event.guild, "ボイスチャンネル 人数制限変更", HexColor.Neutral) { "ボイスチャンネル `${event.channel.name}` の人数制限が `${event.oldUserLimit}` から `${event.channel.userLimit}` に変更されました。" }
    }
    ...
    override suspend fun onGuildBan(event: GuildBanEvent) {
        messageLog(event.guild, "BAN", HexColor.Bad) { "`${event.user.displayName}` がBANされました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUnban(event: GuildUnbanEvent) {
        messageLog(event.guild, "BAN解除", HexColor.Good) { "`${event.user.displayName}` がBAN解除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildAvailable(event: GuildAvailableEvent) {
        messageLog(event.guild, "障害復帰", HexColor.Good) { "このDiscordサーバーは障害から復旧しました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUnavailable(event: GuildUnavailableEvent) {
        messageLog(event.guild, "障害発生", HexColor.Bad) { "このDiscordサーバーでは現在障害が発生しています。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateRegion(event: GuildUpdateRegionEvent) {
        messageLog(event.guild, "リージョン変更", HexColor.Good) { "リージョンが `${event.oldRegion.name} (${event.oldRegionRaw})` から `${event.newRegion.name} (${event.newRegionRaw})` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateName(event: GuildUpdateNameEvent) {
        messageLog(event.guild, "サーバー名変更", HexColor.Neutral) { "サーバー名が `${event.oldName}` から `${event.guild.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateAfkChannel(event: GuildUpdateAfkChannelEvent) {
        messageLog(event.guild, "AFKチャンネル変更", HexColor.Neutral) {
            if (event.oldAfkChannel == null && event.newAfkChannel != null) {
                "AFKチャンネルが ${event.oldAfkChannel.name}` に設定されました。"
            } else if (event.oldAfkChannel != null && event.newAfkChannel == null) {
                "AFKチャンネル ${event.oldAfkChannel.name}` が無効になりました。"
            } else {
                "AFKチャンネルが `${event.oldAfkChannel.name}` から `${event.guild.afkChannel.name}` に変更されました。"
            }
        }
    }
    ...
    override suspend fun onGuildUpdateSystemChannel(event: GuildUpdateSystemChannelEvent) {
        messageLog(event.guild, "システムチャンネル変更", HexColor.Neutral) { "システムチャンネルが `${event.oldSystemChannel.name}` から `${event.guild.systemChannel.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateOwner(event: GuildUpdateOwnerEvent) {
        messageLog(event.guild, "オーナー変更", HexColor.Neutral) { "オーナーが `${event.oldOwner.fullNameWithoutGuild}` から `${event.guild.owner.fullNameWithoutGuild}` に譲渡されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateVerificationLevel(event: GuildUpdateVerificationLevelEvent) {
        messageLog(event.guild, "認証レベル変更", HexColor.Neutral) { "認証レベルが `${event.oldVerificationLevel.name}` から `${event.guild.verificationLevel.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateNotificationLevel(event: GuildUpdateNotificationLevelEvent) {
        messageLog(event.guild, "標準の通知設定変更", HexColor.Neutral) { "標準の通知設定が `${event.oldNotificationLevel.name}` から `${event.guild.defaultNotificationLevel.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateExplicitContentLevel(event: GuildUpdateExplicitContentLevelEvent) {
        messageLog(event.guild, "不適切な表現のフィルター変更", HexColor.Neutral) { "不適切な表現のフィルターが `${event.oldLevel}` から `${event.guild.explicitContentLevel.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateMFALevel(event: GuildUpdateMFALevelEvent) {
        messageLog(event.guild, "サーバーの二段階認証変更", HexColor.Neutral) { "サーバーの二段階認証が `${event.oldMFALevel.name}` から `${event.guild.requiredMFALevel.name}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateSplash(event: GuildUpdateSplashEvent) {
        messageLog(event.guild, "スプラッシュ変更", HexColor.Neutral) { "スプラッシュが `${event.oldSplashUrl}` から `${event.guild.splashUrl}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateAfkTimeout(event: GuildUpdateAfkTimeoutEvent) {
        messageLog(event.guild, "AFKタイムアウト変更", HexColor.Neutral) { "AFKタイムアウトが `${event.oldAfkTimeout.seconds / 60}分` から `${event.guild.afkTimeout.seconds / 60}分` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateFeatures(event: GuildUpdateFeaturesEvent) {
        messageLog(event.guild, "機能変更", HexColor.Neutral) { "機能が `${event.oldFeatures.joinToString(", ")}` から `${event.guild.features.joinToString(", ")}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildUpdateIcon(event: GuildUpdateIconEvent) {
        messageLog(event.guild, "アイコン変更", HexColor.Neutral) { "アイコンが `${event.oldIconUrl}` から `${event.guild.iconUrl}` に変更されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onEmoteAdded(event: EmoteAddedEvent) {
        messageLog(event.guild, "絵文字追加", HexColor.Good) { "絵文字 ${event.emote.asMention} `${event.emote.name}` が追加されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onEmoteRemoved(event: EmoteRemovedEvent) {
        messageLog(event.guild, "絵文字削除", HexColor.Bad) { "絵文字 ${event.emote.asMention} `${event.emote.name}` が削除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onEmoteUpdateName(event: EmoteUpdateNameEvent) {
        messageLog(event.guild, "絵文字名前変更", HexColor.Neutral) { "絵文字の名前が :${event.newName}: `:${event.oldName}:` → `:${event.newName}:` に変更されました。" }
    }
    ...
    override suspend fun onGuildMemberJoin(event: GuildMemberJoinEvent) {
        messageLog(event.guild, "メンバー追加", HexColor.Good) { "${event.member.fullNameWithoutGuild} ${event.member.asMention} がサーバーに参加しました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onGuildMemberLeave(event: GuildMemberLeaveEvent) {
        messageLog(event.guild, "メンバー退出", HexColor.Bad) { "`${event.member.fullNameWithoutGuild}` がサーバーから退出しました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onRoleDelete(event: RoleDeleteEvent) {
        messageLog(event.guild, "ロール削除", HexColor.Bad) { "ロール `${event.role.name}` が削除されました。" }
    }

    @Event(priority = Priority.Lowest)
    override suspend fun onRoleUpdateName(event: RoleUpdateNameEvent) {
        if (event.oldName == "new role") {
            messageLog(event.guild, "ロール追加", HexColor.Good) { "ロール `${event.role.name}` が作成されました。" }
        } else {
            messageLog(event.guild, "ロール名前変更", HexColor.Neutral) { "ロールの名前が `${event.oldName}` から `${event.role.name}` に変更されました。" }
        }
    }
    ...
    override suspend fun onGuildMemberRoleAdd(event: GuildMemberRoleAddEvent) {
        ...
        event.roles.forEach {
            ...
            if (it.idLong in watchRoles) {
                messageLog(event.guild, "システムロール 付与", HexColor.Change, slack = {}) { "${event.member.fullNameWithoutGuild} ${event.member.asMention} に `${it.name}` が付与されました。" }
            }
        }
    }
    ...
    override suspend fun onGuildMemberRoleRemove(event: GuildMemberRoleRemoveEvent) {
        ...
        event.roles.forEach {
            ...
            if (it.idLong in watchRoles) {
                messageLog(event.guild, "システムロール 剥奪", HexColor.Change, slack = {}) { "${event.member.fullNameWithoutGuild} ${event.member.asMention} から `${it.name}` が剥奪されました。" }
            }
        }
    }
    ...
    private fun messageLog(guild: Guild, title: String, color: HexColor, slack: (() -> Unit)? = null, message: () -> String) {
