...
class MessageCaching(bot: GLaDOS): ListenerFeature(bot) {
            ...
            event.jda.guilds.forEach { guild ->
                guild.textChannels.filter { it.hasLatestMessage() }.forEach { channel ->
                    ...
                    if (self != null && self.hasPermission(Permission.MESSAGE_HISTORY)) {
                        bot.logger.info { "テキストチャンネル: #${channel.name} の取得を開始します." }
                        channel.getMessages(300).forEach {  m ->
                            bot.messageCacheManager.add(m)
                            m.attachments.forEach {
                                val file = GLaDOS.getTmpFile("attachments", m.guild.id, m.channel.id, "${m.id}_${it.idLong}_${it.fileName}")
                        ...
                        bot.logger.warn { "テキストチャンネル: #${channel.name} (${guild.name}) に対するメッセージ履歴閲覧権限がありません." }
                    }
                }
            }

            bot.logger.info { "テキストチャンネルのキャッシュが完了しました." }
        }
    }
